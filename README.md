...
1. The user files will be stored in an S3 bucket. 

2. There is a URL for that bucket. What should it be?

3. Where do you want to store the FP data? This includes information about the user's outlines, and stats generated by Fargo Publisher. It's also an S3 path. You might not want this to be public, although there is no personal information stored about the user outlines. It's all designed for publishing. These are <i>public</i> outlines, by definition. 

4. What domain for the user names? You have to own the domain and the DNS should point to the node server so it can redirect to the user's content when subdomains are accessed.

For my deployment, for the initial Fargo 2 beta testers, I went with: http://beta.fargo.io/users/.

I am storing the names in http://beta.fargo.io/names/. 
